...
function Badge({ status }) {
  const map = {
    pending:{bg:'#fef4e7',color:'#e67e22'},scheduled:{bg:'#e8f0fb',color:'#0b4f82'},
    completed:{bg:'#e8f7ef',color:'#1a7a4a'},unpaid:{bg:'#fdecea',color:'#c0392b'},
    partial:{bg:'#fef4e7',color:'#e67e22'},paid:{bg:'#e8f7ef',color:'#1a7a4a'}
  }
  const s = map[status] || {bg:'#f0f4f8',color:'#8898aa'}
  return <span style={{display:'inline-block',background:s.bg,color:s.color,fontSize:11,fontWeight:600,padding:'2px 9px',borderRadius:20}}>{status?.toUpperCase()}</span>
}
...
export default function PatientDetail({ navigate, patientId }) {
  ...
  return (
    <div>
      ...
      {/* Orders tables */}
      {[
        {title:'Simulation Orders', items:sim_orders, type:'sim', cols:['Ref','Date requested','Status']},
        ...
        <div key={type} style={{background:'#fff',border:'1px solid #dde3ec',borderRadius:10,marginBottom:12,overflow:'hidden'}}>
          <div style={{padding:'13px 20px',borderBottom:'1px solid #dde3ec',fontWeight:600,fontSize:13.5}}>{title} <span style={{color:'#8898aa',fontWeight:400,fontSize:12}}>({items.length})</span></div>
          {items.length===0
            ? <div style={{padding:'18px 20px',color:'#8898aa',fontSize:13}}>No {title.toLowerCase()} yet.</div>
            : <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead><tr>{cols.map(c=><th key={c} style={{padding:'8px 16px',textAlign:'left',fontSize:10.5,fontWeight:700,color:'#8898aa',textTransform:'uppercase',letterSpacing:'.05em',borderBottom:'1px solid #dde3ec',background:'#fafbfc'}}>{c}</th>)}</tr></thead>
                <tbody>
                  {items.map(item=>(
                    <tr key={item.id}>
                      <td style={{padding:'10px 16px',fontSize:12.5,fontFamily:'monospace',borderBottom:'1px solid #f0f4f8'}}>{item.order_ref}</td>
                      {type==='sim' && <td style={{padding:'10px 16px',fontSize:12.5,color:'#4a5a70',borderBottom:'1px solid #f0f4f8'}}>{fmtDate(item.sim_date_requested)}</td>}
                      {type==='clinical' && <>
                        <td style={{padding:'10px 16px',fontSize:12.5,color:'#4a5a70',borderBottom:'1px solid #f0f4f8'}}>{item.technique||'—'}</td>
                        <td style={{padding:'10px 16px',fontSize:12.5,color:'#4a5a70',borderBottom:'1px solid #f0f4f8'}}>{item.total_dose_gy?item.total_dose_gy+'Gy/'+item.fractions+'F':'—'}</td>
                      </>}
                      {type==='estimate' && <td style={{padding:'10px 16px',fontSize:12.5,color:'#4a5a70',borderBottom:'1px solid #f0f4f8'}}>{item.total_egp?fmtEGP(item.total_egp)+(item.has_tbd?' + TBD':''):'TBD'}</td>}
                      <td style={{padding:'10px 16px',borderBottom:'1px solid #f0f4f8'}}><Badge status={item.status}/></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {billing && (
        <div style={{background:'#fff',border:'1px solid #dde3ec',borderRadius:10,padding:'18px 22px',marginBottom:12}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:payments.length?14:0}}>
