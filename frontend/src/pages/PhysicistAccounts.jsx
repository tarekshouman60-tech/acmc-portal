...
export default function PhysicistAccounts() {
  ...
  const [form, setForm] = useState({full_name:'',email:'',username:'',phone:'',password:''})
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const [editId, setEditId] = useState(null)
  const EMPTY = {full_name:'',email:'',username:'',phone:'',password:''}
  ...
  async function save() {
    ...
    try {
      if (editId) { const d={...form}; if (!d.password) delete d.password; await api.updatePhysicist(editId, d) } else await api.createPhysicist(form)
      const fresh = await api.physicists(); setPhysicists(fresh)
      setShowForm(false); setEditId(null)
      setForm({full_name:'',email:'',username:'',phone:'',password:''})
  ...
  return (
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
        ...
        <button onClick={()=>{setEditId(null);setForm(EMPTY);setError('');setShowForm(true)}} style={{padding:'9px 18px',borderRadius:7,border:'none',background:'#155eef',color:'#fff',cursor:'pointer',fontSize:13,fontWeight:600}}>+ Add Physicist</button>
      </div>
      ...
      {showForm && (
        <div style={{background:'#fff',border:'1px solid #e7ebf1',boxShadow:'0 2px 6px rgba(15,23,42,.06),0 14px 32px -12px rgba(21,94,239,.28)',borderRadius:14,padding:20,marginBottom:14}}>
          <div style={{fontWeight:600,fontSize:13,marginBottom:14}}>{editId?'Edit account':'New Medical Physicist Account'}</div>
        ...
        {physicists.length===0
          ...
          : <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead><tr style={{background:'#f7f9fc'}}>
                {['Name','Username','Email','Mobile','Joined','Status','Action'].map(h=>(
              ...
              <tbody>
                {physicists.map(r=>(
                  <tr key={r.id}>
                    ...
                    <td style={{padding:'11px 16px',fontSize:12.5,color:'#4a5a70',borderBottom:'1px solid #f0f4f8'}}>{r.username||'—'}</td>
                    <td style={{padding:'11px 16px',fontSize:12.5,color:'#4a5a70',borderBottom:'1px solid #f0f4f8'}}>{r.email}</td>
                    <td style={{padding:'11px 16px',fontSize:12.5,color:'#4a5a70',borderBottom:'1px solid #f0f4f8'}}>{r.phone||'—'}</td>
                    ...
                    <td style={{padding:'11px 16px',borderBottom:'1px solid #f0f4f8'}}>
                      <button onClick={()=>{setEditId(r.id);setForm({full_name:r.full_name||'',email:r.email||'',username:r.username||'',phone:r.phone||'',password:''});setError('');setShowForm(true);window.scrollTo(0,0)}} style={{padding:'5px 12px',borderRadius:5,border:'1px solid #dde3ec',background:'#fff',cursor:'pointer',fontSize:12,fontWeight:500,marginRight:6}}>Edit</button>
