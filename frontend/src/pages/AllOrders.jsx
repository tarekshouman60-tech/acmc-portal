...
export default function AllOrders({ navigate, doctorOnly }) {
  const { user } = useAuth()
  ...
  return (
    <div>
      <div style={{marginBottom:20}}>
        <h1 style={{fontSize:22,fontWeight:700}}>{doctorOnly ? 'My Orders' : 'All Orders'}</h1>
      ...
      <div style={{background:'#fff',border:'1px solid #dde3ec',borderRadius:10,overflow:'hidden'}}>
        <table style={{width:'100%',borderCollapse:'collapse'}}>
          <thead><tr style={{background:'#f7f9fc'}}>
            {['Type','Reference','Patient', isAdmin&&!doctorOnly?'Doctor':null,'Date','Status'].filter(Boolean).map(h=>(
          ...
          <tbody>
            {filtered.map(o => {
              ...
              return (
                <tr key={`${o.type}-${o.id}`} style={{borderBottom:'1px solid #f0f4f8'}}>
                  ...
                  {isAdmin && !doctorOnly && <td style={{padding:'10px 16px',fontSize:12.5,color:'#4a5a70'}}>{o.doctor}</td>}
                  <td style={{padding:'10px 16px',fontSize:12,color:'#8898aa'}}>{fmtDate(o.created_at)}</td>
                  <td style={{padding:'10px 16px'}}>
                    {isAdmin
